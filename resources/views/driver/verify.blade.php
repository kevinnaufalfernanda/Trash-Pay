<x-app-layout>
    <x-slot name="header">
        <h2 class="font-serif font-bold text-2xl text-secondary leading-tight">
            {{ __('Input Berat Riil — Pickup #') }}{{ $pickup->id }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="glass-panel rounded-3xl p-6 mb-6">
                <div class="flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <div class="text-sm text-gray-500">Pickup #{{ $pickup->id }} • Est. weight: {{ $pickup->total_weight ?? 'N/A' }} kg</div>
                    </div>
                    <div class="ml-auto">
            ...
            <div class="glass-panel rounded-3xl p-10 relative overflow-hidden"
                ...
                <h3 class="text-3xl font-serif font-bold mb-2 text-secondary tracking-tight">Input Berat Riil</h3>
                <p class="text-gray-500 text-sm mb-8 font-medium">Timbang sampah, pilih kategori, dan koin akan otomatis ditransfer ke akun user.</p>

                <form action="{{ route('driver.verify.store', $pickup->id) }}" method="POST">
                    @csrf

                    <div class="mb-6">
                        <label class="block text-sm font-bold text-gray-700 mb-2">Kategori Terverifikasi</label>
                        <select name="category_id" x-model="categoryId"
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}">{{ $category->icon }} {{ $category->name }} ({{ number_format($category->price_per_kg, 0, ',', '.') }} Koin/kg)</option>
                            @endforeach
                        </select>
                    </div>

                    <div class="mb-10">
                        <label class="block text-sm font-bold text-gray-700 mb-2">Berat Akhir (kg)</label>
                        ...
                        <p class="text-xs text-gray-400 mt-2 text-center">Gunakan timbangan yang akurat.</p>
                    </div>
                    ...
                    <div class="bg-amber-50/50 backdrop-blur-sm rounded-3xl p-8 mb-10 border border-amber-200 text-center relative overflow-hidden">
                        ...
                        <div class="text-sm font-bold tracking-widest text-amber-800 uppercase mb-4">Koin yang akan ditransfer</div>
                        ...
                        <div class="text-sm font-medium text-amber-700 mt-4 bg-white/50 inline-block px-4 py-2 rounded-full border border-amber-200/50">
                            Formula: <strong x-text="weight"></strong> kg ×
                            <strong x-text="selectedCategory ? selectedCategory.price.toLocaleString() : '0'"></strong> Koin/kg
                        </div>
                    </div>
