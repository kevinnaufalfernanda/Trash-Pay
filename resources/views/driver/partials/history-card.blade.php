                ...
                @else
                    @if($order->status === 'completed')
                        <div class="mt-2 text-xs font-bold text-emerald-600 flex items-center gap-1 bg-emerald-50 w-max px-2.5 py-1 rounded-md border border-emerald-100">
                            ...
                            {{ __('Selesai') }} &bull; {{ $order->total_weight }} kg
                        </div>
        ...
        @if($isActive)
            <div class="flex flex-col sm:items-end gap-2 shrink-0" @click.stop>
                <div class="flex gap-2">
                    <a href="{{ route('driver.navigation', $order->id) }}" class="px-5 py-2.5 bg-white/80 border border-gray-200 text-secondary text-sm font-bold rounded-full hover:bg-white transition-colors">{{ __('Navigasi') }}</a>
                    <a href="{{ route('driver.verify', $order->id) }}" class="px-5 py-2.5 bg-gradient-to-r from-primary to-emerald-500 text-white text-sm font-bold rounded-full btn-premium shadow-lg shadow-emerald-500/30">{{ __('Verifikasi Berat') }}</a>
                </div>
                @if($order->cancel_requested_by === 'user')
                ...
                <div class="space-y-5">
                    ...
                    <div class="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                        ...
                        @if($order->address_notes)
                            <div class="text-sm mt-2 pt-2 border-t border-gray-200 font-medium text-gray-600">
                                <span class="text-gray-400">{{ __('Catatan:') }}</span> {{ $order->address_notes }}
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center justify-between p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
                        ...
                        <div class="text-right">
                            <div class="text-xs text-emerald-600 font-bold uppercase tracking-wider mb-1">{{ __('Pelanggan') }}</div>
