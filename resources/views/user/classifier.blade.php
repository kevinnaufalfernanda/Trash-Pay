        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-gray-100" x-data="classifier()">
                <div class="p-8 text-center">
                    ...
                    <h3 class="text-2xl font-bold mb-2">{{ __('Identify Your Waste') }}</h3>
                    <p class="text-gray-500 mb-8">{{ __('Upload a photo of your waste to instantly know its category using our Smart AI.') }}</p>
                    ...
                    <div x-show="!processing && !result" class="max-w-md mx-auto">
                        <label class="flex flex-col items-center px-4 py-6 bg-white rounded-xl shadow-md tracking-wide uppercase border border-blue cursor-pointer hover:bg-emerald-50 hover:text-primary text-emerald-500 ease-linear transition-all duration-150 border-2 border-dashed border-emerald-300">
                            ...
                            <span class="mt-2 text-base leading-normal">{{ __('Select a photo') }}</span>
                    ...
                    <div x-show="processing" class="py-12" style="display: none;">
                        ...
                        <h4 class="text-xl font-semibold text-gray-700 animate-pulse">{{ __('Processing with AI...') }}</h4>
                        <p class="text-gray-500 text-sm mt-2">{{ __('Analyzing shape, texture, and materials.') }}</p>
                    </div>
                    ...
                    <div x-show="result" class="py-8" style="display: none;">
                        ...
                        <h4 class="text-2xl font-bold text-secondary mb-2">{{ __('Category Found!') }}</h4>
                        <div class="text-3xl font-bold text-primary mb-6" x-text="result"></div>
                        
                        <div class="flex justify-center gap-4">
                            <button @click="reset" class="px-6 py-2 bg-gray-200 text-gray-700 rounded-full font-semibold hover:bg-gray-300 transition-colors">{{ __('Scan Another') }}</button>
                            <a href="{{ route('user.pickup') }}" class="px-6 py-2 bg-primary text-white rounded-full font-semibold hover:bg-emerald-600 transition-colors shadow-md">{{ __('Request Pickup') }}</a>
                        </div>
                    </div>
