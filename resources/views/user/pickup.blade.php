        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
                            ...
                            @foreach($categories as $category)
                                <div>
                                    ...
                                    <label for="cat_{{ $category->id }}"
                                        ...
                                        <div class="text-xs text-primary mt-1 font-bold">{{ number_format($category->price_per_kg, 0, ',', '.') }} Koin/kg</div>
                                    </label>
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="mb-10">
                        <div class="flex items-center justify-between mb-2">
                            <div class="flex items-center gap-3">
                                ...
                                <label for="address" class="text-lg font-serif font-bold text-secondary">Alamat Penjemputan</label>
                            </div>
                            <button type="button" @click="getLocation()" 
                                ...
                                <span x-show="!gettingLocation">📍 Deteksi Lokasi Saya</span>
                        ...
                        <div class="mb-4">
                            ...
                            <p class="text-xs text-gray-500 mt-2 font-medium">💡 Anda bisa menggeser pin atau klik pada peta untuk menyesuaikan lokasi akurat.</p>
                        </div>
                        ...
                        <textarea name="address" id="address" rows="2" x-model="addressText"
                               ...
                               placeholder="Contoh: Jl. Sudirman No. 12, Malang" required></textarea>
                               
                        <label for="address_notes" class="text-sm font-bold text-gray-500 uppercase tracking-widest mt-4 block mb-2">Detail Rumah (Opsional)</label>
                        <input type="text" name="address_notes" id="address_notes"
                               ...
                               placeholder="Contoh: Rumah cat hijau tua, pagar hitam">
                    </div>
            ...
            <div class="mt-8 glass-panel rounded-3xl p-8 relative overflow-hidden border-2 border-emerald-100/50 shadow-lg shadow-emerald-500/5">
                ...
                <h3 class="text-2xl font-serif font-bold mb-6 text-secondary flex items-center gap-2">
                    Antrean Penjemputan Anda <span class="animate-pulse">🚚</span>
                </h3>
                <div class="space-y-4">
                    ...
                    <div class="bg-white/60 border border-white/80 rounded-2xl p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm hover:shadow-md transition-all">
                        <div class="flex items-center gap-5">
                            ...
                            <div>
                                ...
                                <div class="text-sm text-gray-500 font-medium">Kategori: {{ $pickup->category->name }} • Est. {{ $pickup->total_weight }} kg</div>
                            </div>
                        </div>
                        <div class="text-left md:text-right">
                            ...
                            @elseif($pickup->status === 'on-the-way')
                                <span class="inline-block px-4 py-2 bg-emerald-50 text-emerald-700 rounded-full text-sm font-bold tracking-wide border border-emerald-200 shadow-sm">🚚 Driver Menuju Lokasi</span>
                                <div class="text-xs text-secondary mt-2 font-bold bg-white/50 inline-block px-3 py-1 rounded-lg border border-white/60">Driver: {{ $pickup->driver->name ?? 'Eco-Driver' }}</div>
                            @endif
                        </div>
                    </div>
                    @endforeach
