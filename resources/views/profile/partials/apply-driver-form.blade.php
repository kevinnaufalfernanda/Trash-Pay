<section>
    <header>
        <h2 class="text-lg font-medium text-secondary font-serif">
    ...
    @if($application)
        <div class="mt-6 p-4 rounded-xl border border-gray-200 {{ $application->status === 'pending' ? 'bg-amber-50' : ($application->status === 'approved' ? 'bg-emerald-50' : 'bg-red-50') }}">
    ...
    @else
        <form method="post" action="{{ route('profile.apply-driver') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
            @csrf
